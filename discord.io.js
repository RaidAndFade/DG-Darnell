(function discordio(Discord){
	...
	DCP.editUserInfo = function(input, callback) {
		...
		for (var key in input) {
			if (plArr.indexOf(key) < 0) return handleErrCB("'" + key + "' is not a valid key. Valid keys are: " + plArr.join(", "));
	...
	DCP.uploadFile = function(input, callback) {
		...
		if (isBuffer) if (input.filename) file = input.file; else return handleErrCB("uploadFile requires a 'filename' value to be set if using a Buffer", callback);
	...
	DCP.acceptInvite = function(inviteCode, callback) {
		if (this.bot) return handleErrCB("This account is a 'bot' type account, and cannot use 'acceptInvite'. Please use the client's inviteURL property instead.", callback);
	 ...
	 * @arg {Number} [channelObj.codec] - The Opus codec you wish to use, 0 is node-opus, 1 is CJOpus. Defaults to 0.
	 * @arg {Boolean} [channelObj.stereo] - Sets the audio to be either stereo or mono. Defaults to stereo.
	 */
	DCP.getAudioContext = function(channelObj, callback) {
		...
		var Opus, opusEncoder, override = encoders[channelObj.codec];
		...
		try {
			Opus = require(override || encoders[0]);
		} catch(e) {
			try {
				Opus = require(encoders[1]);
			} catch(e2) { return handleErrCB("Could not require CJOpus: " + e2, callback); }
		}

		opusEncoder = new Opus.OpusEncoder( 48000, audioChannels );

		voiceSession.audio = voiceSession.audio || new AudioCB(voiceSession, opusEncoder, audioChannels);
		callback(null, voiceSession.audio);
	};
	...
	function handleWSOpen(opts) {
		var ident = {
			"op":2,
			"d": {
				"token": (opts.bot?"Bot ":"")+this.internals.token,
				"v": GATEWAY_VERSION,
	...
	function handlevWSMessage(voiceSession, vMessage) {
		var client = this, vData = JSON.parse(vMessage), callback = voiceSession.callback;
		switch (vData.op) {
	...
	function handlevWSClose(voiceSession) {
		...
		return voiceSession.emitter = null;
	}
	...
	function AudioCB(voiceSession, opusEncoder, audioChannels) {
		...
		} catch(e) {return handleErrCB(e, voiceSession.callback); }

		var bHandleIncomingAudio = handleIncomingAudio.bind(this);
		...
		this.on('removeListener', function(event) {
			if (isFinalIncomingEvListener(this, event)) {
				this._vUDP.removeListener('message', bHandleIncomingAudio);
			}
		});
	}
	...
	function handleIncomingAudio(msg) {
		var header = msg.slice(0, 12),
			...
			enc = this._speakers[ssrc] || new this._opusEncoder.constructor( 48000, this._audioChannels ),
			decrypted;

		if (!this._speakers[ssrc]) this._speakers[ssrc] = enc;
		header.copy(nonce);
		...
		try {
			decrypted = new Buffer(
				nacl.secretbox.open(
					...
					new Uint8Array(nonce),
					new Uint8Array(this._secretKey)
				)
			);

			this.emit('incoming', ssrc, Buffer(enc.decode(decrypted)) );
		} catch (e) { }
	}
	...
	function Channel(client, server, data) {
		...
		copyKeys(data, this, ['permission_overwrites']);
